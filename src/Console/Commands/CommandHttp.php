...
class CommandHttp
{
    public function server(string|null $host = null, string|null $port = null)
    {
        ...
        if ($port) {
            if ((int)$port != $port or !is_numeric($port)) {
                ...
                return;
            } else if (!$this->checkPort($host, $port)) {
            ...
            while ($this->checkPort($host, $port) == false) {
        ...
        Shell::green('web development server enabled on')->normal(sprintf('http://%s:%s', $host, $port))->br()->br();

        $command = sprintf('php -S %s:%s "%s"', $host, $port, path()->framework('Collections/WebServer.php'));
