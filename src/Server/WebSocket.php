...
class WebSocket
{
    ...
    public function clientStorage(int|null $id = null, int|null $timeout = null)
    {
        ...
        if ($this->server->isEstablished($id)) {
            $path = path()->storage('cache/servers/' . $this->pidMaster() . '/' . $id);
    ...
    {
        $path = path()->storage('cache/servers/' . $this->pidMaster() . '/server');
    ...
    public function pidMaster()
    {
        return $this->server->getMasterPid();
    }

    public function pidWorker()
    {
        return $this->server->getWorkerPid();
    }
}
